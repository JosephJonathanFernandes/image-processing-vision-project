...
if uploaded_file is not None:
    ...
    elif operation == "Overlay Edges on Original":
        edges = cv2.Canny(gray, 100, 200)
        edges_colored = cv2.cvtColor(edges, cv2.COLOR_GRAY2RGB)
        result = cv2.addWeighted(img_np, 0.8, edges_colored, 0.2, 0)

    elif operation == "Invert Image":
        result = cv2.bitwise_not(img_np)

    elif operation == "Resize":
        width = st.slider("New Width", 50, img_np.shape[1]*2, img_np.shape[1])
        height = st.slider("New Height", 50, img_np.shape[0]*2, img_np.shape[0])
